...
static const rclcpp::Logger LOGGER = rclcpp::get_logger("servo_subscriber");
...
class ServoSubscriber : public rclcpp::Node
{
  ...
  private:
    ...
    void topic_callback(const trajectory_msgs::msg::JointTrajectory::SharedPtr msg) const
    {
      ...
      send_goal_options.goal_response_callback = [&](std::shared_future<GoalHandle::SharedPtr> future)
        {
          ...
          if (!goal_handle) {
            RCLCPP_DEBUG(LOGGER, "Goal rejected");
          }
          else {
            RCLCPP_DEBUG(LOGGER, "Goal accepted");
          }
        };
      ...
      send_goal_options.result_callback = [&](const GoalHandle::WrappedResult & result)
        {
          RCLCPP_DEBUG(
            LOGGER, "common_result_response time: %f",
          ...
          switch (result.code) {
            ...
            case rclcpp_action::ResultCode::CANCELED:
              ...
              return;
            default:
              RCLCPP_DEBUG(LOGGER, "Unknown result code");
              return;
          }
        };
  ...
  private:
    ...
    // {
    //   RCLCPP_DEBUG(
    //     LOGGER, "common_goal_response time: %f", rclcpp::Clock().now().seconds());
    //   const auto goal_handle = future.get();
    //   return;
    // }
