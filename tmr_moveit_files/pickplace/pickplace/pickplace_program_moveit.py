...
from pickplace_msgs.srv import AskModbus, AskMoveIt
...
def moveit_set_position(node, moveit, position):
    pos = AskMoveIt.Request()
    pos.coordinates = position
    future = moveit.call_async(pos)
    rclpy.spin_until_future_complete(node, future)
...
def main():
    ...
    moveit = pickplace_node.create_client(AskMoveIt, 'pickplace/moveit')
    
    pickplace_node.get_logger().info("TESTTESTETESTSETSTESTES")
    ...
    msg = MoveCube()
    pickplace_node.get_logger().info("starting loop now")

    try:
        while True:
            
            pickplace_node.get_logger().info("moving to view pick")
            moveit_set_position(pickplace_node, moveit ,view_pick)
            ...
            moveit_set_position(pickplace_node, moveit, safepick)
            
            pickplace_driver.open()
            moveit_set_position(pickplace_node, moveit, pick)
            pickplace_driver.close()
            change_marker("EOAT", pick, flagpublisher, msg)
            moveit_set_position(pickplace_node, moveit, safepick)

            moveit_set_position(pickplace_node, moveit, view_place)
            place, safeplace = get_positions(pickplace_driver, modbus, tf, "vbase_place", vjob_name)
            call_set_parameters(pickplace_node, place)
            moveit_set_position(pickplace_node, moveit, safeplace)
            moveit_set_position(pickplace_node, moveit, place)
            pickplace_driver.open()
